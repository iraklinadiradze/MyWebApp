...
import { CountryViewParams, CountryView } from '../country-entity';
...
/** CountrytList component*/
export class CountryListComponent implements IDynamicComponent {
    inputParams: any;
    userContext: any;

    countryViewParams: CountryViewParams;
    ...
    public gridData: CountryView[];

  constructor(private countryService: CountryService) {
    this.countryViewParams = new CountryViewParams();
    this.countryService.getCountryView(this.countryViewParams).subscribe(r => { this.gridData = r });
  }
...
//    this.compState = ComponentState.EditItem;
  ...
  onFetchDataEnd(e) {
    ...
    if (result.Action == "OK") {
      ...
      this.countryViewParams = JSON.parse(e).Params;
      this.countryService.getCountryView(this.countryViewParams).subscribe(r => { this.gridData = r });
      this.compState = ComponentState.ShowData;
    }
    else {
  ...
  public onSelect($event) {
    if ($event.selectedRows.length == 0) this.currentID = -1
      ...
      this.currentID = $event.selectedRows[0].dataItem.country.id;
