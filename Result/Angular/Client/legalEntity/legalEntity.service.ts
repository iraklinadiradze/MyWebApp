...
import { LegalEntityViewParams, LegalEntityView } from '../legalEntity-entity';
import { LegalEntityService } from '../legalEntity.service';
import { FormActionType } from '../../../common/FormActionTypes';

@Component({
    selector: 'app-legalEntity-list',
    templateUrl: './legalEntity-list.component.html',
    styleUrls: ['./legalEntity-list.component.scss']
})
/** LegalEntitytList component*/
export class LegalEntityListComponent implements IDynamicComponent {
    inputParams: any;
    userContext: any;

    legalEntityViewParams: LegalEntityViewParams;
    ...
    public gridData: LegalEntityView[];

  constructor(private legalEntityService: LegalEntityService) {
    this.legalEntityViewParams = new LegalEntityViewParams();
    this.legalEntityService.getLegalEntityView(this.legalEntityViewParams).subscribe(r => { this.gridData = r });
  }
...
//    this.compState = ComponentState.EditItem;
  ...
  onFetchDataEnd(e) {
    ...
    if (result.Action == "OK") {
      ...
      this.legalEntityViewParams = JSON.parse(e).Params;
      this.legalEntityService.getLegalEntityView(this.legalEntityViewParams).subscribe(r => { this.gridData = r });
      this.compState = ComponentState.ShowData;
    }
    else {
  ...
  public onSelect($event) {
    if ($event.selectedRows.length == 0) this.currentID = -1
      ...
      this.currentID = $event.selectedRows[0].dataItem.legalEntity.id;
