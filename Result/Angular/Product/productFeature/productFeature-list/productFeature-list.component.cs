...
import { ProductFeatureViewParams, ProductFeatureView } from '../productFeature-entity';
import { ProductFeatureService } from '../productFeature.service';
import { FormActionType } from '../../../common/FormActionTypes';

@Component({
    selector: 'app-productFeature-list',
    templateUrl: './productFeature-list.component.html',
    styleUrls: ['./productFeature-list.component.scss']
})
/** ProductFeaturetList component*/
export class ProductFeatureListComponent implements IDynamicComponent {
    inputParams: any;
    userContext: any;

    productFeatureViewParams: ProductFeatureViewParams;
    ...
    public gridData: ProductFeatureView[];

  constructor(private productFeatureService: ProductFeatureService) {
    this.productFeatureViewParams = new ProductFeatureViewParams();
    this.productFeatureService.getProductFeatureView(this.productFeatureViewParams).subscribe(r => { this.gridData = r });
  }
...
//    this.compState = ComponentState.EditItem;
  ...
  onFetchDataEnd(e) {
    ...
    if (result.Action == "OK") {
      ...
      this.productFeatureViewParams = JSON.parse(e).Params;
      this.productFeatureService.getProductFeatureView(this.productFeatureViewParams).subscribe(r => { this.gridData = r });
      this.compState = ComponentState.ShowData;
    }
    else {
  ...
  public onSelect($event) {
    if ($event.selectedRows.length == 0) this.currentID = -1
      ...
      this.currentID = $event.selectedRows[0].dataItem.productFeature.id;
