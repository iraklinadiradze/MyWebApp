...
import { ProductViewParams, ProductView } from '../product-entity';
...
export class ProductListComponent implements IDynamicComponent {
    ...
    public gridData: ProductView[];

  constructor(private productService: ProductService) {
    this.productViewParams = new ProductViewParams();
    this.productService.getProductView(this.productViewParams).subscribe(r => { this.gridData = r });
  }
...
//    this.compState = ComponentState.EditItem;
  ...
  onFetchDataEnd(e) {
    ...
    if (result.Action == "OK") {
      ...
      this.productViewParams = JSON.parse(e).Params;
      this.productService.getProductView(this.productViewParams).subscribe(r => { this.gridData = r });
      this.compState = ComponentState.ShowData;
    }
    else {
  ...
  public onSelect($event) {
    if ($event.selectedRows.length == 0) this.currentID = -1
      ...
      this.currentID = $event.selectedRows[0].dataItem.product.id;
